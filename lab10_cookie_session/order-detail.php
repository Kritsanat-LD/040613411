...
<html>
    <body>
        <?php
            $stmt = $pdo->prepare("SELECT member.name,orders.ord_id,ord_date,pname,pdetail,quantity,price from member 
            ...
            if($row = $stmt->fetch()==null){
                echo 'ลูกค้าท่านนี้มไ่มีการสั่งซื้อ';
            }

            while ($row = $stmt->fetch()) {
                if (!$username) {
                    $username = $row["name"];
                    echo "<h3>ชื่อสมาชิก : $username </h3>";
                }
